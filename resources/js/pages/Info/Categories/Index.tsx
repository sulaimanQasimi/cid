...
import { CanDelete, CanUpdate, CanView } from '@/components/ui/permission-guard';
...
import { AlertTriangle, Eye, Pencil, Trash, TrendingUp } from 'lucide-react';
...
export default function InfoCategoriesIndex({
    ...
    const goToPage = (page: number) => {
        router.get(route('info-categories.index'), { ...filters, page }, { preserveState: true, preserveScroll: true });
    };

    // Apply filters to the URL
    const applyFilters = (newFilters: Partial<Props['filters']>) => {
        router.get(route('info-categories.index'), { ...filters, ...newFilters, page: 1 }, { preserveState: true, preserveScroll: true });
    };
                ...
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        <AlertDialogCancel onClick={() => setIsDeleteDialogOpen(false)}>
                            {t('info_categories.delete_dialog.cancel')}
                        </AlertDialogCancel>
            ...
            <div className="container px-0 py-6">
                ...
                <div className="mt-8">
                    <SearchFilters
                        ...
                        className="overflow-hidden bg-gradient-to-bl from-white to-purple-50/30 shadow-2xl dark:from-gray-800 dark:to-purple-900/20"
                    />
                                    ...
                                    <TableHeader>
                                        <TableRow className="border-0 bg-gradient-to-l from-purple-100 to-purple-200 dark:from-purple-900 dark:to-purple-800">
                                            <TableHead className="px-6 py-6 text-lg font-bold text-purple-800 dark:text-purple-200">
                                                {t('info_categories.table.id')}
                                            </TableHead>
                                            <TableHead className="px-6 py-6 text-lg font-bold text-purple-800 dark:text-purple-200">
                                                {t('info_categories.table.name')}
                                            </TableHead>
                                            <TableHead className="px-6 py-6 text-lg font-bold text-purple-800 dark:text-purple-200">
                                                {t('info_categories.table.description')}
                                            </TableHead>
                                            <TableHead className="px-6 py-6 text-lg font-bold text-purple-800 dark:text-purple-200">
                                                {t('info_categories.table.created_at')}
                                            </TableHead>
                                            <TableHead className="px-6 py-6 text-right text-lg font-bold text-purple-800 dark:text-purple-200">
                                                {t('info_categories.table.actions')}
                                            </TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {categories?.data && categories.data.length > 0 ? (
                                            categories.data.map((category: InfoCategory) => (
                                                <TableRow
                                                    key={category.id}
                                                    className="border-b border-purple-100 transition-colors duration-300 hover:bg-purple-50/50 dark:border-purple-800 dark:hover:bg-purple-900/30"
                                                >
                                                        ...
                                                        <div className="flex items-center justify-end gap-2">
                                                            <CanView model="info_category">
                                                                <Button
                                                                    ...
                                                                    className="h-10 w-10 rounded-xl text-blue-600 transition-all duration-300 hover:scale-110 hover:bg-blue-100 hover:text-blue-700 dark:text-blue-400 dark:hover:bg-blue-900/30 dark:hover:text-blue-300"
                                                                >
                                                            ...
                                                            <CanUpdate model="info_category">
                                                                <Button
                                                                    ...
                                                                    className="h-10 w-10 rounded-xl text-green-600 transition-all duration-300 hover:scale-110 hover:bg-green-100 hover:text-green-700 dark:text-green-400 dark:hover:bg-green-900/30 dark:hover:text-green-300"
                                                                >
                                                            ...
                                                            <CanDelete model="info_category">
                                                                <Button
                                                                    ...
                                                                    className="h-10 w-10 rounded-xl text-red-600 transition-all duration-300 hover:scale-110 hover:bg-red-100 hover:text-red-700 dark:text-red-400 dark:hover:bg-red-900/30 dark:hover:text-red-300"
                                                                >
                                                ...
                                                <TableCell colSpan={5} className="h-32 text-center">
                                                    <div className="flex flex-col items-center gap-4 text-purple-600 dark:text-purple-400">
                                                        ...
                                                        <p className="text-purple-500 dark:text-purple-400">
                                                            {t('info_categories.no_records_description')}
                                                        </p>
                                                    </div>
                                                </TableCell>
                                            </TableRow>
                ...
                {categories?.links && categories.links.length > 0 && (
                    <div className="mt-8 flex justify-center">
                        <div className="rounded-3xl border border-purple-200 bg-gradient-to-l from-purple-50 to-white p-4 shadow-2xl dark:border-purple-800 dark:from-purple-900/30 dark:to-gray-800">
