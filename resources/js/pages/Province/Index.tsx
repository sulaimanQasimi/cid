          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:justify-between lg:items-center gap-8">
            <div className="flex items-center gap-8">
              ...
              <div className="space-y-3">
                <h2 className="text-4xl lg:text-5xl font-bold text-white drop-shadow-2xl tracking-tight">{t('provinces.page_title')}</h2>
                <div className="text-white/90 flex items-center gap-3 text-xl font-medium">
                  <div className="p-2 bg-white/20 rounded-xl backdrop-blur-sm">
                    <BarChart3 className="h-6 w-6" />
                  </div>
                  {t('provinces.page_description')}
                </div>
              </div>
            </div>
          ...
          <CardHeader className="py-4 bg-gradient-to-l from-purple-500 to-purple-600 text-white cursor-pointer" onClick={() => setIsFiltersOpen(!isFiltersOpen)}>
            <CardTitle className="text-lg font-semibold flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xl font-bold">{t('provinces.search_filters')}</div>
                  <div className="text-purple-100 text-xs font-medium">{t('provinces.find_and_filter')}</div>
                </div>
              </div>
                ...
                <div>
                  ...
                  >
                    <SelectTrigger className="h-11 shadow-lg border-purple-200 focus:border-purple-500 focus:ring-purple-500/20 bg-gradient-to-l from-purple-50 to-white rounded-xl text-sm">
                      <SelectValue placeholder={t('provinces.sort_by')} />
                    </SelectTrigger>
                    <SelectContent>
                      {sortOptions.map(option => (
                        <SelectItem key={option.value} value={option.value}>
                          {t(`provinces.sort_options.${option.value}`)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <div className="md:col-span-2 lg:col-span-4 grid grid-cols-3 gap-3">
                  ...
                  <div>
                    <Button
                      ...
                      title={t(`provinces.sort_${filters.direction === 'asc' ? 'ascending' : 'descending'}`)}
                    ...
                    >
                      ...
                      {filters.direction === 'asc' ? t('provinces.sort_ascending') : t('provinces.sort_descending')}
                    </Button>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger className="h-11 shadow-lg border-purple-200 focus:border-purple-500 focus:ring-purple-500/20 bg-gradient-to-l from-purple-50 to-white rounded-xl text-sm">
                        <SelectValue placeholder={t('provinces.per_page_option')} />
                      </SelectTrigger>
                      <SelectContent>
          ...
          <Card className="shadow-2xl overflow-hidden bg-gradient-to-bl from-white to-purple-50/30 border-0 rounded-3xl">
            <CardHeader className="bg-gradient-to-l from-purple-500 to-purple-600 text-white py-6">
              <CardTitle className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-2xl font-bold">{t('provinces.table.title')}</div>
                  <div className="text-purple-100 text-sm font-medium">{t('provinces.table.description')}</div>
                </div>
              </CardTitle>
            </CardHeader>
                  ...
                  <TableBody>
                    {provinces?.data && provinces.data.length > 0 ? (
                      provinces.data.map((province: ProvinceData) => (
                        <TableRow key={province.id} className="hover:bg-purple-50/50 transition-colors duration-300 border-b border-purple-100">
                          ...
                          <TableCell className="font-bold text-purple-900 py-6 px-6 text-lg">{province.code}</TableCell>
                          <TableCell className="text-purple-800 py-6 px-6 font-medium">
                            {province.capital || '-'}
                          </TableCell>
                          <TableCell className="py-6 px-6">
                            <Badge variant={province.status === 'active' ? 'default' : 'secondary'} className="text-sm font-semibold">
                              {province.status}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-purple-800 py-6 px-6 font-medium">
                      ...
                      <TableRow>
                        <TableCell colSpan={6} className="h-32 text-center">
                          <div className="flex flex-col items-center gap-4 text-purple-600">
                            ...
                            <p className="text-purple-500">{t('provinces.no_records_description')}</p>
                          </div>
                        </TableCell>
                      </TableRow>
                    )}
